...
if is_pid_alive "$pid"; then
  ...
  echo "ops-proxy:UP pid=${pid} url=http://${OPS_MOCK_HOST}:${OPS_MOCK_PORT} http=${code} log=${LOG_FILE}"
else
  echo "ops-proxy:DOWN url=http://${OPS_MOCK_HOST}:${OPS_MOCK_PORT} log=${LOG_FILE}"
  exit 1
fi
